...
func DrawBuildings(screen *ebiten.Image, grid *world.Grid, buildings []*building.Building, cam *Camera, unstaffed, disconnected, highlighted map[*building.Building]bool) {
	...
	for _, b := range standingBuildings {
		...
		case building.Armory:
			drawBuildingBody(screen, b, assets.Armory, sx, sy, tilePixels)
...
// identical on screen. A player watching an enemy serf walk up to some
// warehouse had no way to tell it apart from their own, and reasonably
// read it as "the enemy is delivering to MY warehouse". Owner 0 (the
